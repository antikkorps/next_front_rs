...
import {
  ForgotPassForm,
  ResetPassForm,
  forgotPassFormSchemaValidation,
  resetPasswordSchemaValidation,
} from "@/zod/auth/forgot-password";
...
export const checkTokenEmailVerification = async (token: string) => {
  const tCheckConfirmation = await getTranslations("Resend_Confirmation_Mail");
  const response = await fetch(
    `${API_ENDPOINTS.CHECK_CONFIRMATION_MAIL_TOKEN}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!data.success) {
    if (data.error && data.message === "User already confirmed") {
      return {
        error: { message: tCheckConfirmation("check.already_confirmed") },
      };
    }
    return { error: { message: tCheckConfirmation("check.missing_token") } };
  }
  if (data.success) {
    return {
      success: { message: tCheckConfirmation("check.confirmation_success") },
    };
  }
  return data;
};
